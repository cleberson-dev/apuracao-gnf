  ...
  <section class="flex-grow-[3] h-[100svh] shadow-lg px-10 py-4 dark:text-white/80 flex flex-col justify-center"
    ...
    <div class="flex items-start mb-2">
      <div class="flex-grow text-[#2E3790]">
        ...
        <p class="m-0 font-normal text-sm block text-[#909090]" v-if="formattedLatestUpdate">
      ...
      <div class="text-right font-bold flex flex-col mb-4">
        <div>
          ...
          <h2 class="text-[#909090] text-xs">
            Votos apurados ({{
              formatNumbers(sectionStore.votosApurados) }}/{{ formatNumbers(sectionStore.totalElectors) }})
          </h2>
        </div>
        <div class="text-right">
          <h1 class="text-primary text-[2rem]">
            {{ formatarPercentual(sectionStore.closedSections.length / sectionStore.allSections.length) }}%
          </h1>
          <h2 class="text-[#909090] text-base">
            Seções totalizadas ({{ sectionStore.closedSections.length }}/{{ sectionStore.allSections.length }})
          </h2>
        </div>
      </div>
    </div>
