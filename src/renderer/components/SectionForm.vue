...
const classes = {
  input:
    "border border-solid border-borderColor focus:outline-primary placeholder:text-black p-2 text-sm rounded uppercase w-full",
};
...
<template>
  <FormKit
    type="form"
    :classes="{ form: 'flex flex-col gap-4 mt-10', message: 'text-red-500' }"
    @submit="onSubmit"
    :plugins="[castRangeToNumber]"
    :submit-label="section ? 'Atualizar' : 'Criar'"
    :submit-attrs="{
      inputClass:
        'bg-primary w-full text-white rounded p-2 hover:brightness-90 transition-all',
    }"
  >
    <div class="grid grid-cols-3 gap-4">
      ...
      <FormKit
        ...
        type="number"
        label="Nº de Eleitores"
        name="voters"
        :value="(section?.voters ?? 0) + ''"
    ...
    <FormKit
      ...
      :classes="{
        input: classes.input,
        message: 'text-red-500',
        wrapper: 'flex gap-1 items-center w-fit',
      }"
      type="checkbox"
      label="Totalizada"
      name="closed"
    ...
    >
      ...
      <ul class="flex flex-col gap-2">
        ...
        >
          <span
            class="inline-block rounded-full size-4 mr-1"
          ...
          <FormKit
            ...
            :classes="{
              input:
                'border border-solid border-borderColor outline-primary w-16 p-2 rounded font-semibold ml-2',
              message: 'text-red-500 absolute left-0 -bottom-3',
            }"
            :name="candidate.number + ''"
            :value="section.votes[candidate.number] + ''"
            validation="required|min:0"
          />
        </li>
      </ul>
