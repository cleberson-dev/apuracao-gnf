<template>
  <div class="relative" @blur="onBlur" ref="container-ref">
    ...
    >
      <input
        type="text"
        placeholder="Selecione sua seção"
    ...
    >
      <li v-if="filteredItems.length === 0" class="p-2 text-center">
      ...
      <li
        ...
        :class="{
          ...
          'bg-green-100 hover:bg-green-200': item.marked,
        }"
...
const selected = computed(() =>
  props.value ? filteredItems.value.find((item) => item.value === props.value) : undefined
);
...
const clickOutside = (e: MouseEvent) => {
  ...
  const exactMatch = filteredItems.value.find(
    (v) => v.label.toLowerCase() === searchText.value.toLowerCase()
  );
  if (exactMatch) {
...
const onKeyDown = (e: KeyboardEvent) => {
  ...
  if (e.key === "ArrowDown") {
    ...
    const idx = filteredItems.value.findIndex(
      (val) => val.value === hovered.value!.value
    );
    if (idx === -1) return;
